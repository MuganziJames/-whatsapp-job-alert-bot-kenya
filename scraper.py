...
# MyJobMag scraper removed to improve performance and reduce processing time
# The async Playwright-based scraper was slow and provided minimal unique value
# Keeping only BrighterMonday (primary) and Jobs Kenya (fast backup) for optimal speed
...
def scrape_jobs_multi_source(interest: str, max_jobs: int = 15) -> List[Dict[str, Any]]:
    ...
    # MyJobMag scraper removed to improve performance and reduce processing time
    # Only using BrighterMonday (primary) and Jobs Kenya (fast backup) for optimal speed
